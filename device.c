...
static struct Device *init_device(__reg("a6") struct ExecBase *sys_base, __reg("a0") BPTR seg_list, __reg("d0") struct Device *device)
{
    ...
    /* Initialize message lists and mutexes */
    ...
    /* Initialize hardware, ethernet channel, fast speed */
    spi_initialize(&ctx->spi, SPI_CHANNEL_ETHERNET, SysBase);
    spi_set_speed(&ctx->spi, SPI_SPEED_FAST);
    nic_init(&ctx->spi);
